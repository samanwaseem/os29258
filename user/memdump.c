...
memdump(char *fmt, char *data)
{
    for (char *f = fmt; *f; f++) {
        ...
        case 'c': { // print the next 1 byte of the data as an 8-bit ASCII character
            char val = *data;
            printf("%c\n", val);
            data += 1;
            break;
        }
        case 's': { //the next 8 bytes of the data contain a 64-bit pointer to a C string; print the string
            char *s = *(char **)data;
            printf("%s\n", s);
            data += 8;
            break;
